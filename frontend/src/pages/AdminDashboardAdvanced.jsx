        ...
        <Box sx={{ width: '100%', px: { xs: 1, sm: 1.5, md: 2 }, py: 2 }}>
            ...
            {activeTab === 0 && (
                <Box sx={{ animation: 'fadeIn 0.5s ease-in-out' }}>
                    ...
                    <Grid container spacing={0} sx={{ mb: 4, gap: 2 }}>
                        {[
                            { label: 'Tổng Reviews', value: stats?.totalReviews || 0, color: '#2563eb', icon: <AssessmentOutlined /> },
                            { label: 'Đã Huấn Luyện', value: stats?.usedForTraining || 0, color: '#10b981', icon: <CheckCircleOutlined /> },
                            { label: 'Chờ Xử Lý', value: stats?.unusedForTraining || 0, color: '#f59e0b', icon: <WarningOutlined /> },
                            { label: 'Pneumonia / Normal', value: `${stats?.pneumoniaCount || 0} / ${stats?.normalCount || 0}`, color: '#ef4444', icon: <RateReviewOutlined /> }
                        ].map((item, index) => (
                            <Grid item xs={12} sm={6} md={3} key={index} sx={{ flex: '1 1 auto' }}>
