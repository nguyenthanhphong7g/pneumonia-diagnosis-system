...
export default function Home() {
    ...
    try {
      ...
      if (res.data.error) {
        ...
        setResult({
          ...
          inference_time_ms: res.data.inference_time_ms || 0,
          model_time_ms: res.data.model_time_ms || 0,
          gradcam_time_ms: res.data.gradcam_time_ms || 0,
          timings_ms: res.data.timings_ms || {},
  ...
  const handleCompare = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.error || 'So sánh mô hình thất bại');
    } finally {
      setLoading(false);
    }
  };
  ...
  // Định nghĩa mô tả cho từng độ đo
  const metricExplanations = {
    '✅ Độ chính xác': 'Tỷ lệ % các dự đoán của mô hình là chính xác. Ví dụ: 92% có nghĩa là mô hình dự đoán đúng 92 trên 100 ca.',
    '🎯 Độ an toàn': 'Trong các trường hợp mô hình dự đoán "mắc bệnh", thực tế mắc bệnh bao nhiêu %. Số càng cao, bệnh nhân càng yên tâm khi được dự đoán mắc bệnh.',
    '🔍 Tỷ lệ phát hiện': 'Trong tất cả các trường hợp thực sự mắc bệnh, mô hình phát hiện được bao nhiêu %. Số càng cao, càng ít bỏ sót bệnh nhân mắc bệnh.',
    '⚖️ Chỉ số cân bằng': 'Chỉ số đo lường sự cân bằng giữa độ an toàn và tỷ lệ phát hiện. Số càng cao, mô hình càng cân bằng.',
    '📊 Chỉ số hiệu năng': 'Khả năng mô hình phân biệt được giữa bệnh nhân mắc và không mắc bệnh. Từ 0 đến 1, số càng gần 1 càng tốt.'
  };
  ...
  const RenderMetricBar = ({ label, value, color }) => {
    ...
    return (
      <Tooltip title={explanation} arrow placement="right" sx={{ cursor: 'help' }}>
            ...
            <Box sx={{ width: '100%', maxWidth: '100%' }}>
              ...
              <Paper elevation={0} sx={{
                ...
                <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} sx={{ mt: 4, alignItems: 'center' }}>
                  ...
                  >
                    {loading ? 'ĐANG PHÂN TÍCH...' : (selectedModel === 'compare' ? 'SO SÁNH CÁC MÔ HÌNH' : 'CHẨN ĐOÁN NGAY')}
                  </Button>
              ...
              {compareResults && (
                <Paper elevation={0} sx={{ p: 4, borderRadius: '28px', border: '1px solid #e2e8f0', boxShadow: '0 10px 30px rgba(0,0,0,0.04)', minWidth: 500 }}>
                  <Typography variant="subtitle2" sx={{ fontWeight: 800, mb: 3, color: '#64748b', textAlign: 'center' }}>
                    SO SÁNH KẾT QUẢ THEO MÔ HÌNH ĐÃ CHỌN
                  </Typography>
                  <Stack spacing={2}>
                      ...
                      <Card key={idx} elevation={0} sx={{ display: 'flex', justifyContent: 'space-between', p: 2, border: '1px solid #f1f5f9', bgcolor: '#f8fafc', borderRadius: '16px' }}>
                        <Box>
                          ...
                          <Typography variant="body2" sx={{ color: m.label === "Pneumonia" ? '#e11d48' : m.label === "Normal" ? '#16a34a' : '#475569', fontWeight: 600 }}>
                            {m.label === "Pneumonia" ? 'Viêm phổi' : m.label === "Normal" ? 'Bình thường' : m.error || '—'}
                          </Typography>
                        </Box>
                        <Box sx={{ textAlign: 'right' }}>
            ...
            <Box sx={{ position: 'sticky', top: 24 }}>
              ...
              <Paper elevation={0} sx={{ p: 3, borderRadius: '24px', border: '1px solid #e2e8f0', boxShadow: '0 4px 20px rgba(0,0,0,0.02)', mb: 3, minWidth: 370 }}>
                ...
                <FormControl fullWidth>
                  <InputLabel id="model-select-label">Chọn mô hình chẩn đoán</InputLabel>
                  <Select
                    labelId="model-select-label"
                    value={selectedModel}
                    label="Chọn mô hình chẩn đoán"
                    onChange={(e) => {
                  ...
                  >
                    ...
                    <MenuItem value="compare">
                      <Typography sx={{ fontWeight: 600, color: '#0369a1' }}>So sánh các mô hình</Typography>
                    </MenuItem>
                  </Select>
              ...
              <Paper elevation={0} sx={{ p: 3, borderRadius: '24px', border: '1px solid #e2e8f0', boxShadow: '0 4px 20px rgba(0,0,0,0.02)' }}>
                ...
                {selectedModel === 'compare' ? (
                  loadingCompareMetrics ? (
                    ...
                    <Box sx={{ mt: 2, overflowX: 'auto' }}>
                      <Box sx={{ minWidth: 320 }}>
                        ...
                        {/* Metric Rows */}
                        {[
                          { label: 'Độ chính xác', key: 'accuracy' },
                          { label: 'Độ an toàn', key: 'precision' },
                          { label: 'Phát hiện', key: 'recall' },
                          { label: 'Cân bằng', key: 'f1Score' },
                          { label: 'Hiệu năng', key: 'auc' }
                        ].map(({ label, key }) => {
                          const metricKey = key.includes('Score') ? key.replace('Score', 'score') : key;
                          ...
                          return (
                            <Box key={label} sx={{ display: 'grid', gridTemplateColumns: '80px 70px 70px 70px', gap: 0.25, py: 0.5, borderBottom: '1px solid #f1f5f9' }}>
                              <Typography variant="caption" sx={{ fontWeight: 600, color: '#475569', fontSize: '0.7rem' }}>{label}</Typography>
                        ...
                        {/* <Box sx={{ display: 'grid', gridTemplateColumns: '80px 70px 70px 70px', gap: 0.25, py: 0.5 }}>
                          ...
                          <Box sx={{ textAlign: 'center', bgcolor: '#f0f9ff', p: 0.25, borderRadius: '4px' }}>
                            <Typography variant="caption" sx={{ fontWeight: 700, color: '#0369a1', fontSize: '0.7rem' }}>
                              {compareMetrics.gated_fusion?.expectedRuntimeMs || '—'} ms
                            </Typography>
                          </Box>
                          <Box sx={{ textAlign: 'center', bgcolor: '#f0fdf4', p: 0.25, borderRadius: '4px' }}>
                            <Typography variant="caption" sx={{ fontWeight: 700, color: '#16a34a', fontSize: '0.7rem' }}>
                              {compareMetrics.vit?.expectedRuntimeMs || '—'} ms
                            </Typography>
                          </Box>
                          <Box sx={{ textAlign: 'center', bgcolor: '#fef3c7', p: 0.25, borderRadius: '4px' }}>
                            <Typography variant="caption" sx={{ fontWeight: 700, color: '#d97706', fontSize: '0.7rem' }}>
                  ...
                  <Box sx={{ mt: 2 }}>
                    <RenderMetricBar label="✅ Độ chính xác" value={metrics.accuracy} color="#3b82f6" />
                    <RenderMetricBar label="🎯 Độ an toàn" value={metrics.precision} color="#10b981" />
                    <RenderMetricBar label="🔍 Tỷ lệ phát hiện" value={metrics.recall} color="#f59e0b" />
                  ...
                  <Box sx={{ textAlign: 'center', py: 4, bgcolor: '#f8fafc', borderRadius: '12px', border: '1px dashed #cbd5e1' }}>
                    <Typography variant="caption" sx={{ color: '#94a3b8', fontWeight: 500 }}>
                      Chưa có dữ liệu độ đo.<br />Vui lòng chọn mô hình khác.
                    </Typography>
                  </Box>
                )}
              </Paper>
