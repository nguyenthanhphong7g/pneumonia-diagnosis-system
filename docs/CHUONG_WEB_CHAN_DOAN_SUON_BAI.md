...
Quy trình chẩn đoán thực tế bao gồm các bước chính sau:

**1. Xác thực và khởi tạo phiên**

- Hệ thống kiểm tra ảnh X-quang: định dạng file (JPEG, PNG), dung lượng (≤5MB) và tính toàn vẹn.
- Nếu hợp lệ, khởi tạo phiên chẩn đoán mới với mã định danh duy nhất (UUID), ghi nhận người dùng, mô hình được chọn, timestamp bắt đầu và version mô hình.
- Nếu không hợp lệ, trả lỗi và dừng quá trình ngay lập tức để tiết kiệm tài nguyên.

**2. Tiền xử lý ảnh**

- Resize ảnh từ kích thước gốc về 224×224 pixels (kích thước đầu vào của ViT và WST).
- Chuẩn hóa giá trị pixel: chia cho 255 để đưa về phạm vi [0, 1] rồi áp dụng ImageNet normalization (mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]).
- Chuyển từ định dạng ảnh RGB sang tensor NumPy với shape (1, 3, 224, 224) để phù hợp với batch processing.
- Điều này đảm bảo tính nhất quán giữa các phiên chẩn đoán, tránh sai lệch do khác biệt trong tiền xử lý.

**3. Trích xuất đặc trưng đa nhánh**

Gated Fusion xử lý tensor ảnh qua ba nhánh song parallel:

| Nhánh | Kiến trúc | Đầu vào | Đầu ra | Mục đích |
|-------|-----------|---------|--------|---------|
| ViT | Vision Transformer | (1,3,224,224) | Vector 768-dim | Nắm bắt thông tin toàn cục, cấu trúc ở mức độ cao |
| WST | Wavelet Scattering Transform | (1,3,224,224) | Vector 512-dim | Trích xuất đặc trưng đa tỉ lệ, các biến đổi cấu trúc chi tiết |
| RadSTA | Radiomics + Statistical Attributes | (1,3,224,224) | Vector 256-dim | Phân tích thống kê, kết cấu, hình thái bệnh lý |

Các vector đặc trưng từ ba nhánh có kích thước khác nhau, nên chúng được projection về không gian chung (512 chiều) trước khi hợp nhất.

**4. Cơ chế Gate - Điều chỉnh động trọng số các nhánh**

Đây là bước cốt lõi của Gated Fusion. Thay vì ghép đặc trưng cố định từ ba nhánh, hệ thống tính toán vector gate động:

- Nhập: ba vector đặc trưng đã được projection (kích thước bằng nhau, 512-dim).
- Tính toán gate coefficient cho mỗi nhánh thông qua fully connected layer: $g_i = \sigma(W_i \cdot f_i + b_i)$ (với $\sigma$ là activation function).
- Chuẩn hóa gate weights bằng softmax: $\tilde{g}_i = \frac{e^{g_i}}{\sum_{j=1}^{3} e^{g_j}}$ để đảm bảo tổng bằng 1 và thể hiện đóng góp tương đối.
- Hợp nhất vector fusion: $f_{fusion} = \sum_{i=1}^{3} \tilde{g}_i \cdot f_i$.

Ý nghĩa thực tế: với mỗi ảnh cụ thể, hệ thống tự quyết định nên tin tưởng nhánh nào hơn. Ví dụ, nếu ảnh có cấu trúc phức tạp, ViT có thể nhận trọng số cao (e.g., 0.6); nếu có dấu hiệu bệnh rõ ràng ở các tỉ lệ khác nhau, WST được ưu tiên (e.g., 0.5). Điều này khác biệt so với các mô hình kết hợp đơn giản (simple concatenation) vì nó mang tính **thích ứng động** với từng ảnh đầu vào.

**5. Phân loại và tính độ tin cậy**

- Vector fusion (512-dim) được đưa qua fully connected classifier layer: $\text{logits} = W_{clf} \cdot f_{fusion} + b_{clf}$.
- Tính xác suất cho hai lớp bằng softmax: $P(\text{NORMAL}) = \frac{e^{logits[0]}}{e^{logits[0]} + e^{logits[1]}}$, $P(\text{PNEUMONIA}) = 1 - P(\text{NORMAL})$.
- Chọn nhãn có xác suất cao nhất. Độ tin cậy được định nghĩa là $\max(P(\text{NORMAL}), P(\text{PNEUMONIA}))$ (thường trong khoảng 0.5 đến 1.0).
- Đồng thời, hệ thống nắp bắt gradient của logits đối với các pixel ảnh để sinh Grad-CAM: $\text{CAM} = \text{ReLU}\left(\sum_c w_c \cdot A_c\right)$, trong đó $A_c$ là activation map của lớp cuối cùng trước classifier, $w_c$ là gradient trung bình.

**6. Lưu trữ kết quả và truy vết**

Backend nhận kết quả từ AI service và lưu vào cơ sở dữ liệu:
- **Cột chẩn đoán**: `user_id`, `diagnosis_id`, `created_at`, `class_label` (NORMAL/PNEUMONIA).
- **Cột độ tin cậy**: `confidence` (0.0-1.0), `confidence_normal` (xác suất lớp NORMAL), `confidence_pneumonia`.
- **Cột mô hình**: `model_id`, `model_version` (ví dụ: "gated_fusion_v2.1"), `model_weights_hash` (checksum để kiểm tra version chính xác).
- **Cột thời gian xử lý**: `inference_time_ms` (thời gian từ khi nhận ảnh đến khi tính toán xong, thường 200-500ms).
- **Cột file**: `image_path` (đường dẫn ảnh gốc trên server), `gradcam_path` (đường dẫn ảnh Grad-CAM).
- **Metadata khác**: `gate_weights` (lưu trữ giá trị $\tilde{g}_1, \tilde{g}_2, \tilde{g}_3$ dưới dạng JSON để kiểm tra hành vi mô hình).

Nhờ lưu `gate_weights`, hệ thống có thể phân tích sau này: "Trong những ca bệnh nặng, nhánh nào được ưu tiên hơn?" hoặc "Liệu trọng số gate có ổn định theo thời gian?", hỗ trợ đánh giá độ tin cậy và tính ổn định của mô hình Gated Fusion trong triển khai thực tế.

**Khi nào gọi model và được gì**

Trong web application, Gated Fusion được gọi trong các tình huống cụ thể sau:

| Tình huống | Kích hoạt | Output | Mục đích sử dụng |
|-----------|----------|--------|-----------------|
| **Chẩn đoán đơn lẻ** | Người dùng tải 1 ảnh + bấm "Chẩn đoán ngay" | Nhãn (NORMAL/PNEUMONIA), confidence (e.g., 0.92), Grad-CAM | Cung cấp kết quả chẩn đoán trực tiếp cho người dùng |
| **So sánh mô hình** | Người dùng bấm "So sánh các model" | Kết quả từ Gated Fusion + 2-3 mô hình khác song song | Đánh giá hiệu suất tương đối, xác nhận Gated Fusion tốt hơn |
| **Kiểm tra lại lịch sử** | Người dùng mở bản ghi cũ, click "Tính toán lại" | Kết quả mới từ model hiện tại + so sánh với kết quả cũ | Phát hiện sai khác, kiểm tra độ ổn định theo thời gian |
| **Xuất báo cáo** | Người dùng bấm "Export PDF" trên 1 phiên chẩn đoán | PDF chứa ảnh gốc, Grad-CAM, confidence, gate_weights | Tạo hồ sơ bệnh án, lưu chứng cứ y tế |

**Lợi ích khi gọi model:**

1. **Kết quả dự đoán chuyên sâu** - Nhãn chẩn đoán không chỉ là "NORMAL" hoặc "PNEUMONIA" mà kèm theo độ tin cậy (0.5-1.0) để bác sĩ đánh giá độ ổn định của kết quả.

2. **Giải thích kết quả** - Grad-CAM cho thấy vùng ảnh nào mô hình tập trung, giúp bác sĩ xác minh liệu model có nhìn đúng vị trí bệnh hay không.

3. **Truy vết hành vi mô hình** - `gate_weights` được lưu lại, cho phép phân tích sau:
   - Ảnh bình thường: ViT có trọng số 0.5, WST 0.3, RadSTA 0.2 (tin vào cấu trúc toàn cục).
   - Ảnh viêm phổi nặng: WST có trọng số 0.6, RadSTA 0.3, ViT 0.1 (tập trung vào chi tiết cấu trúc).
   - Điều này giúp kiểm tra mô hình có "học đúng" các đặc điểm bệnh học hay không.

4. **Hỗ trợ quyết định lâm sàng** - Kết hợp kết quả model với kinh nghiệm của bác sĩ, giảm thời gian chẩn đoán và tăng nhất quán kết quả.

5. **Lưu vết và kiểm định** - Mỗi lần gọi model được ghi lại với timestamp, model_version, gate_weights, cho phép kiểm toán và so sánh hiệu suất model qua các thời kỳ.
