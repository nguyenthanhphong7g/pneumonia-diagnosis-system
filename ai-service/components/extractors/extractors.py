...
try:
    from radiomics import featureextractor as _radiomics_featureextractor
    import SimpleITK as _sitk
    HAVE_PYRADIOMICS = True
except Exception:
    _radiomics_featureextractor = None
    _sitk = None
    HAVE_PYRADIOMICS = False
...
def extract_vit_from_pil(pil_image):
    ...
    # Avoid processor-side normalize edge cases that can produce NaN/Inf.
    # =====================================================
    ...
    pixel_values = np.transpose(img_rgb, (2, 0, 1))[None, ...].astype(np.float32)

    inputs = {
        "pixel_values": torch.from_numpy(pixel_values).to(_device)
    }
...
def extract_radiomics_stats_from_pil(pil_image):
    ...
    if HAVE_PYRADIOMICS:
        try:
            ...
            extractor = _radiomics_featureextractor.RadiomicsFeatureExtractor(
                **params
            )
            ...
            image_itk = _sitk.GetImageFromArray(image)

            mask_itk = _sitk.GetImageFromArray(mask)
